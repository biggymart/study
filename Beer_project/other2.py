...
mp4 = ['video_cocacola1','video_cocacola2',\
        'video_fanta1','video_fanta2'\
        'video_letsbee1','video_letsbee2','video_letsbee3','video_letsbee4',\
        'video_pocari1','video_pocari2',\
        'video_sprite1','video_sprite2',\
        'video_tejava1','video_tejava2','video_tejava3','video_tejava4']

# 영상의 의미지를 연속적으로 캡쳐
for i in mp4 :
    video_file = '../Project01_data/video/' + str(i) + '.mp4'       
    
    vidcap = cv2.VideoCapture(video_file)                            # 영상의 이미지를 연속적으로 캡쳐할 수 있게 하는 class
    count = 0

    while(vidcap.isOpened()):
        ret, image = vidcap.read()
        if not ret :                        # 제대로 프레임을 읽으면 True , 실패하면 False
            break
